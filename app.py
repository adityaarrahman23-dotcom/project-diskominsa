...
# db_master menyimpan format panjang (Vertikal): Kabupaten | Kecamatan | Objek | Nilai
...
with tab1:
    ...
    st.info("Upload file sumber yang memiliki kolom: **Nama Kabupaten/Kota, Nama Kecamatan, Objek, Satuan, Kategori Kerusakan, dan Jumlah**.")
    ...
    if st.button("🚀 Proses Data Mentah", type="primary"):
        if file_mentah:
            ...
            for f in file_mentah:
                ...
                if f.name not in st.session_state.riwayat_file:
                    try:
                        ...
                        if all(col in df.columns for col in kolom_wajib):
                            df_skor = hitung_skor(df)
                            
                            # Ringkas data per file (Group By) termasuk Kabupaten
                            ...
                            st.error(f"Format kolom tidak sesuai pada file: {f.name}. Pastikan ada kolom Kabupaten, Kecamatan, Objek, Kerusakan, dan Jumlah.")
            ...
            if data_baru_mentah:
                ...
                # Group by ulang untuk menyatukan wilayah & objek yang sama
            ...
            for f in file_lama:
                if f.name not in st.session_state.riwayat_file:
                    try:
                        ...
                        st.error(f"Gagal menggabungkan {f.name}: Pastikan file memiliki kolom Kabupaten di kolom pertama dan Kecamatan di kolom kedua.")
...
with tab3:
    if not st.session_state.db_master.empty:
        ...
        col_m1.metric("Total Wilayah (Kecamatan)", len(df_final))
        col_m2.metric("Total Objek Unik", len(st.session_state.db_master['Objek'].unique()))
